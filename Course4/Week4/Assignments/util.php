...
function loadEdus($pdo, $profile_id) {
    // $statement = $pdo->prepare("SELECT Education.profile_id, Institution.name, Education.rank, Education.year FROM education WHERE profile_id = :pid INNER JOIN Institution ON Institution.institution_id = Education.institution_id ORDER BY rank ");
